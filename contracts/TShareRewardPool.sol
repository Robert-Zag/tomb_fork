...
contract TShareRewardPool {
    ...
    modifier onlyOperator() {
        require(
            operator == msg.sender,
            "TShareRewardPool: caller is not the operator"
        );
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(
                ...
                "TShareRewardPool: existing pool?"
            );
        }
    }
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _tshareReward = _generatedReward.mul(pool.allocPoint).div(
                totalAllocPoint
            );
            accTSharePerShare = accTSharePerShare.add(
                _tshareReward.mul(1e18).div(tokenSupply)
            );
        }
        return
            user.amount.mul(accTSharePerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            ...
            pool.accTSharePerShare = pool.accTSharePerShare.add(
                _tshareReward.mul(1e18).div(tokenSupply)
            );
        }
        pool.lastRewardTime = block.timestamp;
